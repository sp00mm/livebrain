...
HAS_API_KEY = bool(
    __import__('keyring').get_password('LiveBrain', 'openai_api_key')
)
...
def _run_query(db, embedder, brain, session, query_text):
    ...
    callbacks = ExecutionCallbacks(
        ...
        on_complete=lambda r: response_holder.append(r),
    ...
    return response, steps, deltas, tool_calls


@requires_api_key
@requires_model
class TestFullQueryExecution:
    def test_full_query_execution(self, db, embedder, indexed_brain):
        ...
        assert len(deltas) > 0
        step_types = [s.step_type for s in steps if s.status != StepStatus.COMPLETED]
...
@requires_api_key
@requires_model
class TestToolCallsTriggered:
    def test_tool_calls_triggered(self, db, embedder, indexed_brain):
        brain, session, _ = indexed_brain
...
@requires_api_key
@requires_model
class TestInteractionTracePersisted:
    def test_interaction_trace_persisted(self, db, embedder, indexed_brain):
        brain, session, _ = indexed_brain
        ...
        tool_names = [t.get('name') or t.get('type') for t in interaction.tools]
...
@requires_api_key
@requires_model
class TestToolCallRecordsPersisted:
    def test_tool_call_records_persisted(self, db, embedder, indexed_brain):
        brain, session, _ = indexed_brain
...
@requires_api_key
@requires_model
class TestFileReferencesCorrect:
    def test_file_references_correct(self, db, embedder, indexed_brain):
        brain, session, _ = indexed_brain
...
@requires_api_key
@requires_model
class TestAIResponsePersisted:
    def test_ai_response_persisted(self, db, embedder, indexed_brain):
        brain, session, _ = indexed_brain
        response, _, _, _ = _run_query(
            db, embedder, brain, session,
...
class TestLinkifySources:
    def test_linkify_sources_with_file_refs(self, qapp):
    ...
    def test_linkify_sources_with_code_content(self, qapp):
        from ui.widgets.chat_feed import AnswerItem
        item = AnswerItem()
        item._file_refs = [
            FileReference(
...
class TestRealEmbedder:
    def test_real_embedder_produces_searchable_vectors(self, db, embedder, indexed_brain):
...
class TestMultipleQueries:
    def test_multiple_queries_share_conversation_context(self, db, embedder, indexed_brain):
        brain, session, _ = indexed_brain
